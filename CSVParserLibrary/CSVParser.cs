  ...
  public class CSVParser : ICSVParser
  {
    ...
    private string[] ParseLineNew(string line)
    {
      ...
      foreach (var ch in line)
      {
        ...
        else if (!ignoreDelimiter && _options.Delimiters.Contains(ch))
        {
          if (sb.Length > 0)
          {
            if (_options.TrimEnable)
            {
              output.Add(sb.ToString().Trim(_options.TrimCharacters));
            }
            else
            {
          ...
          else if (sb.Length == 0 && !_options.IgnoreDelimiters.Contains(prevChar))
          {
            output.Add("");
          }
        }
        ...
        {
          if (!ignoreDelimiter && _options.DiscardCharacters?.Contains(ch) == true)
          {
            prevChar = ch;
            continue;
          }
    ...
    private static object? ParseType(string data, Type type)
    {
      if (type.Name == "Nullable`1")
      {
        ...
        return ParseType(data, baseType);
      }
      if (type.Name == "String")
      {
      ...
      else if (type.IsClass)
      {
        ...
        var builderMethod = type.GetMethod("BuildCsvLine");
        if (builderMethod?.IsStatic == true)
        {
          var newObj = type.GetConstructor(new Type[0]);
          return builderMethod.Invoke(newObj, new[] { data });
        }
      }
      else if (type.IsEnum)
      {
        if (Enum.TryParse(type, data, out object? newEnum))
        {
          return newEnum;
        }
      }
